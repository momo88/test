<?php
  ...
  if($action=="del"){
	  ...
	  if(empty($id) || !is_numeric($id)){
	      ...
	      $query = mysql_query("select sid from xh_title where id=".$id,$conn);
		  
		  
		  mysql_free_result($query);
		  deldir("xh_upload/".$id);
		  mysql_query("delete from xh_title where id=".$id,$conn);
		  mysql_query("delete from xh_question where tid=".$id,$conn);
		  mysql_close($conn);		  
	  ...
	  if(isset($_POST["XHID"]) && is_array($_POST["XHID"]) && count($_POST["XHID"])>0){
		  ...
		  $query= mysql_query("delete from xh_title where id in(".$tids.")",$conn);
		  $query= mysql_query("delete from xh_question where tid in(".$tids.")",$conn);
		  mysql_close($conn);
  ...
  else if(isset($_POST["operate"]) && trim($_POST["operate"])=="search"){//搜索投票问题
      mysql_close($conn);
